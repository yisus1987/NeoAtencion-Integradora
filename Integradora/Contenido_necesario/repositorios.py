...
from modelos import Usuario, Paciente, Actividad, ResultadoTest
...
class IRepositorioUsuario(ABC):
    ...
    @abstractmethod
    def autenticar(self, identificador: str, contrasena: str) -> Usuario | None: ...
...
class RepositorioUsuario(IRepositorioUsuario):
    ...
    def crear(self, usuario: Usuario) -> int:
        ...
        cur.execute(
            ...
            "VALUES (%s, %s, %s)",
            (usuario.nombre_usuario, usuario.contrasena, usuario.correo))
        self._conexion.commit()
    ...
    def autenticar(self, identificador: str, contrasena: str) -> Usuario | None:
        cur = self._conexion.cursor()
        cur.execute(
            "SELECT id, nombre_usuario, correo_electronico FROM Usuario "
            "WHERE (correo_electronico = %s OR nombre_usuario = %s) AND contrasena = %s",
            (identificador, identificador, contrasena))
        fila = cur.fetchone()
        cur.close()
        if fila:
            return Usuario(id=fila[0], nombre_usuario=fila[1], correo=fila[2], contrasena="", rol=self._rol(fila[0]))
        return None
...
class RepositorioTutor:
    ...
    def crear(self, datos: dict, id_usuario: int) -> None:
        ...
        cur.execute(
            ...
            "VALUES (%s, %s, %s, %s, %s, %s)",
            (datos.get("nombre"), datos.get("apellido"), datos.get("correo_electronico"), datos.get("telefono"), datos.get("parentesco"), id_usuario)
        )
        self._conexion.commit()
        cur.close()
...
class RepositorioDocente:
    ...
    def crear(self, datos: dict, id_usuario: int) -> None:
        ...
        cur.execute(
            ...
            "VALUES (%s, %s, %s, %s, %s, %s)",
            (datos.get("nombre"), datos.get("apellido"), datos.get("correo_electronico"), datos.get("telefono"), datos.get("nombre_escuela"), id_usuario)
        )
        self._conexion.commit()
        cur.close()
...
class RepositorioEspecialista:
    ...
    def listar(self) -> list[tuple]:
        cur = self._conexion.cursor()
        cur.execute("SELECT id, nombre, descripcion FROM Especialista")
        filas = cur.fetchall()
        cur.close()
        return filas
...
class RepositorioPaciente:
    ...
    def crear(self, paciente: Paciente) -> None:
        cur = self._conexion.cursor()
        cur.execute(
            "INSERT INTO Paciente (nombre, apellido, edad, Nivel_escolar, id_tutor_padre, cuestionario, grupo_TDAH, nivel_TDAH) "
            "VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
            (paciente.nombre, paciente.apellido, paciente.edad, paciente.nivel_escolar, paciente.id_tutor_padre, paciente.cuestionario, paciente.grupo_TDAH, paciente.nivel_TDAH)
        )
        self._conexion.commit()
        cur.close()
    ...
    def actualizar_resultados(self, id_paciente: int, cuestionario: str, grupo: str, nivel: str) -> None:
        cur = self._conexion.cursor()
        cur.execute(
            "UPDATE Paciente SET cuestionario = %s, grupo_TDAH = %s, nivel_TDAH = %s WHERE id = %s",
            (cuestionario, grupo, nivel, id_paciente)
        )
        self._conexion.commit()
        cur.close()

    def guardar_resultado_test(self, resultado: ResultadoTest) -> None:
        cur = self._conexion.cursor()
        cur.execute(
            "INSERT INTO resultado_test (id_Paciente, fecha, puntaje_inatencion, puntaje_hiperactividad, grupos_TDAH, nivel_TDAH) "
            "VALUES (%s, %s, %s, %s, %s, %s)",
            (resultado.id_paciente, resultado.fecha, resultado.puntaje_inatencion, resultado.puntaje_hiperactividad, resultado.grupos_TDAH, resultado.nivel_TDAH)
        )
        self._conexion.commit()
        cur.close()

    def obtener_resultados_paciente(self, id_paciente: int) -> list[ResultadoTest]:
        cur = self._conexion.cursor()
        cur.execute("SELECT id_Paciente, fecha, puntaje_inatencion, puntaje_hiperactividad, grupos_TDAH, nivel_TDAH, id FROM resultado_test WHERE id_Paciente = %s ORDER BY fecha DESC", (id_paciente,))
        filas = cur.fetchall()
        cur.close()
        return [ResultadoTest(id_paciente=f[0], fecha=f[1], puntaje_inatencion=f[2], puntaje_hiperactividad=f[3], grupos_TDAH=f[4], nivel_TDAH=f[5], id=f[6]) for f in filas]
...
class RepositorioAgenda:
    ...
    def por_paciente(self, id_paciente: int) -> list[tuple]:
        cur = self._conexion.cursor()
        cur.execute("""
            SELECT act.nombre_actividad, aa.fecha, esp.nombre
            FROM agenda_actividad aa
            JOIN Actividades act ON aa.id_actividad = act.id
            JOIN Especialista esp ON aa.id_especialista = esp.id
